...
class RegexParser(private val lexer: RegexLexer) {
	...
	private fun S1() : Tree = when(lexer.token) {
		...
		else -> throw ParseException.expectedNotFound(lexer, TOKENS.SELECT, TOKENS.KLEENE, TOKENS.LPAREN, TOKENS.RPAREN, TOKENS.LETTER)
	}
	...
	private fun C1() : Tree = when(lexer.token) {
		...
		else -> throw ParseException.expectedNotFound(lexer, TOKENS.LPAREN, TOKENS.LETTER, TOKENS.KLEENE, TOKENS.SELECT, TOKENS.RPAREN)
	}
